<?php
  ...
  function eliminaTercMiscelaneo($db){
    ...
    if ($elimina->rowCount() == 1){
      $descripcion = utf8_encode("Se eliminó registro de liqterceromisc $nroDoc");
  ...
  function eliminaOtrosGastos($db){
    ...
    if ($tipo == 'Vale'){
      ...
      if ($elimina->rowCount() == 1){
        $descripcion = utf8_encode("Se eliminó registro de combustible $fchCreacion-$hraCreacion-$idPlaca");
      ...
      if ($elimina->rowCount() != 1){
        ...
        if ($elimina->rowCount() == 1){
          $descripcion = utf8_encode("Se eliminó registro de ccmovimientos $nroOrden-$correlativo");
          logAccion($db,$descripcion, NULL, NULL);
        }
        ...
        if ($monto == $montoEncab){
          ...
          if($elimina->rowCount() == 1){
            $descripcion = utf8_encode("Se eliminó registro de ccmovimencab $nroOrden");
            logAccion($db,$descripcion, NULL, NULL);
          }
          return  $elimina->rowCount();
        } else {
          ...
          if($actualiza->rowCount() == 1){
            $descripcion = utf8_encode("Se actualizó registro de ccmovimencab $nroOrden. A $montoEncab se le restó $monto");
            logAccion($db,$descripcion, NULL, NULL);
          }
          return $actualiza->rowCount();
        }
  ...
  function elimDocIdentidad($db){
    ...
    if($elimina->rowCount() > 0){
      $descripcion = utf8_encode("Se eliminó el documento de identidad $nroDocTrab de tipo $tipoDocTrab y con fchInicio $fchIni. Este documento pertenecía al trabajador $idTrabajador");
  ...
  function eliminaPunto($db){
    ...
    return  $elimina->rowCount();
    if ($elimina->rowCount() == 1){
      $descripcion = utf8_encode("Se eliminó un punto: $idPunto - $tipoPunto, $nombClientePunto, $distritoPunto");
      logAccion($db,$descripcion, NULL, NULL);
    }
    return  $elimina->rowCount();
  }
  ...
  function editaDespacho($db){
    ...
    /*
    aquí tendría que ir todo un tema de validaciones
    */
    ...
    /*
      ...
      Se tendría que validar que solo cambie características generales del viaje como kilometraje recorrido, horas de inicio y finde todo el viaje, pero ya no cambia info de la tripulación.
  ...
  function eliminaTrip($db){
    ...
    if ($elimina->rowCount() == 1){
      $descripcion = utf8_encode("Se eliminó registro de despachopersonal fch Despacho: $fchDespacho, correlativo: $correlativo, idTrabajador: $idTrabajador, tipoRol: $tipoRol");
      logAccion($db,$descripcion, NULL, NULL);
    }
    return  $elimina->rowCount();
  }
  ...
  function buscarInfoParaCerrarForzoso($db, $dataPost){
    ...
    $cadenaValTrip = utf8_encode("<thead><tr><th>Trabajador</th><th>Tipo Rol</th><th>Fch. Inicio</th><th>Hr. Inicio</th><th>Fch Fin</th><th>Hr. Fin</th><th>Val Rol</th><th>Val Adic</th><th>Val Hr. Extra</th><th>Acción</th></tr></thead>");
    $cadenaValTrip .= "<tbody>";
    ...
    $cadenaValTrip .= utf8_encode("<tfoot><tr><th>Trabajador</th><th>Tipo Rol</th><th>Fch. Inicio</th><th>Hr. Inicio</th><th>Fch Fin</th><th>Hr. Fin</th><th>Val Rol</th><th>Val Adic</th><th>Val Hr. Extra</th><th>Acción</th></tr></tfoot>");
    ...
    ///////////////
    $cadenaGuias = utf8_encode("<table width = '100%'><tr><th width = '250 px'>Guía Porte</th><th>Usuario</th></tr>");
...
//    $placa    = $dataPost["txtCFplaca"];
 //   $hraInicio     = $dataPost["txtCFhraInicio"];
  ...
  function buscarInfoPagosyCobranzas($db, $dataPost){
    ...
    $cadDataIngEgr .=  utf8_encode("<thead><tr><th width = '140px'>Concepto</th><th width = '60px'>Monto</th><th width = '190px' >Info. Proceso</th><th>Observación</th></tr></thead>");
    ...
    $cadOcurrenDesp .= utf8_encode("<tr><th width = '75'>Tipo</th><th width = '75'>SubTipo</th><th width = '260'>Descripción</th><th width = '50'>Mnt Total</th><th width = '50'>Mnt a Distribuir</th><th>Nro. Doc</th></thead> <tbody>"); 
    ...
    $cadOcurrenTerc .= utf8_encode("<tr><th width = '110'>Tipo</th><th width = '250'>Descripción</th><th width = '50'>Mnt Total</th><th width = '50'>Pagado</th><th width = '60'>Fch Pago</th><th>Doc Pago Tercero</th></thead> <tbody>");
    ...
    foreach ($dataOcurrTerc as $key => $value) {
      /*
      Esta parte está en desarrollo, todavía faltan más pruebas*/
      /*
  ...
  function guardarValTripulante($db, $dataPost){
    ...
    if(!is_numeric( $dataPost["txtValTrabValorRol"] )){
      return "Error. El valor rol ingresado no es un número";
    }

    if(!is_numeric( $dataPost["txtValTrabValorAdic"] )){
      return "Error. El valor adicional ingresado no es un número";
    }

    if(!is_numeric( $dataPost["txtValTrabValorHrExtra"] )){
      return "Error. El valor hora extra ingresado no es un número";
    }
...
// =  * 
  ...
  function guardarValidaDespacho($db){
    ...
    print_r($_POST);
    echo "</pre>";*/
    //Los datos de la tripulación. Si se cambió manualmente queda con su valor, en caso contrario se recalcula.  
...
*/
    ...
    if($txtValPrecBase2 > 0){
      ...
      $descripcion = utf8_encode("El despacho $txtValFchDespacho-$txtValCorrelativo no generó documento de cobranza por que el precio era cero. Precio original: $valPrecioBaseOriginal");
      logAccion($db, $descripcion, '', $txtValPlaca);
    }
    ...
    //Actualizar si hay un vehículo de tercero involucrado
...
$consulta = $db->prepare("SELECT `fchDespacho`, des.`correlativo`, `idProgramacion`, `cli`.rznSocial, concat(des.`idCliente`,'-',`cli`.rznSocial ) AS rznCliente , `guiaCliente`, `hraInicio`, `fchDespachoFinCli`, `fchDespachoFin`, `hraFin`, `placa`, `considerarPropio`, `m3`, `valorServicio`, `igvServicio`, des.`idCliente`, `cuenta`, `correlCuenta`, `tipoServicioPago`, des.`nroAuxiliares`, `nroAuxiliaresAdic`, `costo`, `topeServicioHraNormal`, des.`hrasNormales`, des.`tolerHrasNormales`, `costoHraExtra`, `toleranCobroHraExtra`, `valorConductor`, `hraNormCondDespacho`, `tolerHraCondDespacho`, `valHraAdicCondDespacho`, `valorAuxiliar`, `hraNormAuxDespacho`, `tolerHraAuxDespacho`, `valHraAdicAuxDespacho`, `nroAuxiliaresCuenta`, `valorAuxAdicional`, `usarMaster`, `tpoExtraHras`, `costoTotal`, `nroGuias`, `nroDespachos`, `usaReten`, `concluido`, `hraInicioBase`, `kmInicio`, `kmInicioCliente`, `kmFinCliente`, `hraFinCliente`, `lugarFinCliente`, `kmFin`, `recorridoEsperado`, `zonaDespacho`, `idZonaDespacho`, `tolerHraIniEsperadoDesp`, `tolerHraFinEsperadoDesp`, `valAuxTerceroDesp`, `puntosDesp`, `ptoOrigen`, `idOrigen`, `idSedeOrigen`, `tipoDestino`, `ptoDestino`, `observacion`, `observCliente`, `docCobranza`, `tipoDoc`, `pagado`, `pagoEnPlanilla`, `terceroConPersonalMoy`, des.`contarDespacho`, des.`superCuenta`, des.`usuario`, des.`fchCreacion`, `modoCreacion`, `estadoDespacho`, `usuarioGrabaFin`, `fchGrabaFin`, `hraGrabaFin`, `usuarioCreaCobranza`, `fchCreaCobranza`, des.`idProducto`, clicuepro.`nombProducto`, des.`valKmAdic`, des.`hraIniEsperado`, des.`valAdicHraIniEsper`, des.`hraFinEsperado`, des.`valAdicHraFinEsper`, des.`cobrarPeaje`, des.`cobrarRecojoDevol`, des.`valUnidTercCCond`, des.`valUnidTercSCond`, des.`hrasNormalTerc`, des.`tolerHrasNormalTerc`, des.`valHraExtraTerc`, des.`valKmAdicTerc`, des.`tolerKmEsperado`, des.`movilAsignado`, `editaUsuarioDesp`, `usuarioAsignado`, `modoTerminado`, `modoConcluido`, `fchTerminado`, `usuarioTerminado`  FROM `despacho` AS des, cliente AS cli, clientecuentaproducto AS clicuepro WHERE `des`.idProducto = `clicuepro`.idProducto AND `des`.idCliente = `cli`.idRuc AND des.`fchDespacho` = :fchDespacho AND des.`correlativo` = :correlativo ");
    ...
    */
    ...
    //$idxTrab = 'idTrabajador'.$i;
      ...
      if (!empty($porcentaje)){
        ...
        $actualiza = $db->prepare("UPDATE `ocurrenciaconsulta` SET `descripcion` = replace(`descripcion`,'&#209;','Ñ'),   descripcion = replace(descripcion,'&#241;','ñ')  WHERE `fchDespacho` = :fchDespacho AND  `correlativo` = :correlativo AND  `tipoOcurrencia` = :tipoOcurrencia ");
        $actualiza->bindParam(':fchDespacho',$fchDespacho);
        $actualiza->bindParam(':correlativo',$correlativo);
        $actualiza->bindParam(':tipoOcurrencia',$tipoOcurrencia);
        $actualiza->execute();
  ...
  function validarPasarEstadoAnterior($db){
    ...
    if($auxData["cant"] == 0 ) $rptaVehTer = "Puede proceder";
      ...
      $rptaVehTer = utf8_encode("Pago vehículo tercero tramitado, no puede proceder");
      $deshabilitar = "Si";
    }
