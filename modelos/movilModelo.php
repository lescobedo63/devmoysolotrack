...
//Ya no se usa???
  ...
  function verificarDatosKilometraje($db, $idProgramacion, $kmInicio, $kmInicioCliente, $kmFinCliente, $kmFin){
    ...
    */
    $consulta = $db->prepare("SELECT `fchDespacho`, `correlativo`, `idProgramacion`, placa, `concluido`, `modoCreacion`, `estadoDespacho`, kmUltimaMedicion FROM `despacho` LEFT JOIN vehiculo ON `vehiculo`.idPlaca = `despacho`.placa WHERE idProgramacion = :idProgramacion ");
    $consulta->bindParam(':idProgramacion',$idProgramacion);
    $consulta->execute();
    $auxData = $consulta->fetch();
